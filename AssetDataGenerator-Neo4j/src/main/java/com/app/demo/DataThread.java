...
public class DataThread {
	...
	public void populateAsset() {
		...
		for (int i = 1; i <= DBUtils.numberOfAsset; i++) {
			...
			Asset asset = new Asset(i, name, stock);
	...
	public void populateCustomer() {
		...
		for (int i = 1; i <= DBUtils.numberOfCustomer; i++) {
			...
			Customer customer = new Customer(i,name);
	...
	public void populatePortfolio() {
		long start = System.currentTimeMillis();
		for (int i = 1; i <= DBUtils.numberOfPortfolio; i++) {
			int customerId = i;
			int assetId = df.getNumberBetween(1, DBUtils.numberOfAsset);
			String portfolioName = "P_" + df.getLastName() + "_" + i;
			Portfolio portfolio = new Portfolio(i,portfolioName,assetId);
			portfolioDAO.create(portfolio);
		}
		long time = System.currentTimeMillis() - start;
		printMsg("Portfolio", DBUtils.numberOfPortfolio, time);
	}
